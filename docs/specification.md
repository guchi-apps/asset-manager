...
**12項目目（データ取得状況・#276）を足した時点で、この「ちょうど収まる」は超えている。** 内訳は
ヘッダー64px＋メニュー620px＋フッター79px＝約763pxで、実効745pxに対して約18px超える。
`SidebarContent` は `flex-1 overflow-auto` なのでスクロールはでき、末尾の「設定」が半分見えた状態に
なるため「まだ下がある」ことは読み取れる。**次に項目を足すときは1行ぶんまるごと隠れる**ので、
グループの整理か、既存ページ配下への格納を先に検討すること。
