...
| Zaim側の最終更新が記録日より前 | 保存せずスキップして通知する（`detectStaleSource`） |
...
の `isStaleForDay`）。連携していない口座（現金・手入力）は最終更新を持たないため対象外。
1件でも古い行を合算したまとまりは、合計そのものが古いとみなしてスキップする。
...
- `/api/zaim/sync` は従来どおり無条件に上書きし、異常値・鮮度の検知も行わない（呼び出し側が結果を確認できるため）。ただし記録日の決め方は同じで、応答の `recordDayKey` で確認できる
