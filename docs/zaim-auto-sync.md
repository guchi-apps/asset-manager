...
`recordDayKey` は実際に保存した日（JST）で、**叩いた日とは限らない**。記録日はAIDEが巡回した
時刻から決まるため（後述）、巡回前に叩けば前日の記録になる。
...
作らない。ログの `記録日:` 行に、その実行がどの日へ書いたかが出る。
...
| Zaim側の最終更新が記録日より前 | 保存せずスキップする（`detectStaleSource`）。通知は1件も保存できなかった日だけ |
