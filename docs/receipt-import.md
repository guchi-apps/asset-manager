...
現行実装は品目を入れているが**出金元が「反映待ち」口座**なので、作成経路より先にそこを疑う。

| | 内容 | これが通れば |
| --- | --- | --- |
| A | 反映待ち口座＋品目ありのAPI明細（現行実装と同じ） | 現行のまま運用できる。実装変更なし |
| B | 連携カード口座＋品目ありのAPI明細（Aから出金元だけを変えたもの） | 出金元を変えるだけで済む |

どちらも候補に出なければ、**API登録の明細は置き換えに載せられない**と結論できる。その場合は
Web版の品目入力（AIDE側のPlaywright実装が要る）か、置き換えを使わない運用へ切り替える。
