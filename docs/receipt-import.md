...
| Zaimから読み込む | 内訳の提案タブ | 内訳が未設定の直近60日の支出（`refreshGenreSuggestions`）。Zaimは変えない | 提案タブを使うたび |
...
| 状態 | 意味 |
| --- | --- |
| `CONFIRMED` | 確定済み。反映待ち口座へ登録できる |
| `SENT_TO_ZAIM` | 反映待ち口座へ登録済み。Zaimアプリでの置き換え待ち |
...
- **② 反映待ち（登録済み）の明細は合わせない。** Zaimへ登録済みの明細の日付を書き換える口がAIDEに無いため。
...
「明細」タブは、状態ごとのカードを縦に並べるのをやめ、**「① 確認 → ② 反映待ち」**の
手順で並べている（#431 では「③ 反映済み」もあったが、#456 で外した）。
DBの状態（`ReceiptStatus`）は増やしておらず、対応は `lib/receipt-flow.ts` の
`receiptFlowStep` だけで決める（一覧・詳細画面・テストが同じ対応を見る）。

| 手順 | 状態 | 行で押す操作 |
| --- | --- | --- |
| ① 確認 | `REVIEW_REQUIRED` / `CONFIRMED` / `ANALYZING` | 「登録」「修正」「削除」 |
| ② 反映待ち | `SENT_TO_ZAIM` | 「置き換えた」（押すと一覧から外れる） |
| 画面に出さない | `REPLACED` | なし（#456） |
| 手順の外（赤帯） | `MANUAL_ACTION_REQUIRED` / `FAILED` | 詳細画面で対応する |

- **「反映待ち」はもともと手順の名前で、Zaimの「反映待ち」口座とは別概念だった。** #464からは
  登録先そのものが「反映待ち」口座になったため、両者は同じものを指すようになった
  （前述「登録先は『反映待ち』口座に固定する」）
- **「登録」は確定と反映待ち口座への登録を1回で行う**（`confirmAndSendReceipt`）。判定は
...
- 押せるかどうかは `registerBlocker` が押す前に判定し、押せない理由を行に出す。条件は上の2関数が
  弾くもの（検算・内訳・購入日・店舗名・反映待ち口座の有無・AIDE設定）に揃えてあるので、
  **どちらかの条件を変えたらここも直す**
- 反映したかどうかは機械では判定しない（前述「置き換え待ち」）。代わりに置き換えの手がかりと、
  登録からの経過日数（JSTの暦日。`WAITING_STALE_DAYS` 以上で色を変える）、
  Web版の一覧に候補があるか（#443。「連携明細あり / 見つからない / 確認できない」）を出す。
  ①確認の手順にも同じ候補を出す（#451。まだ登録前なので文言だけ変えている）

## 突合せタブ（Issue #456）

「突合せ」タブは、**Zaimのカード連携明細**（AIDEが巡回したWeb版の一覧）と、① 確認・② 反映待ちの
...
- **① 確認の明細で「重複の可能性」（#445）に出たZaim明細とは組にしない**（#451 の `excludeDismissedAsDuplicate` と同じ規則）。
...
Zaimアプリでの手入力と、複数の経路から記録されることがある。「明細」タブの ① 確認・② 反映待ちの
明細に、同じ支払いと思われる相手を並べる。**消すかどうかは人が決める**（自動削除・自動統合はしない）。
判定は `lib/receipt-duplicates.ts`（純粋関数）、DBとZaim APIの読み出しは `lookupReceiptDuplicates`。

| 相手 | 読み方 |
| --- | --- |
| 取り込み同士（置き換え済みを含む） | DB |
| Zaimに記録済みの明細（手入力・置き換え済み） | Zaim公式APIの `GET /v2/home/money`。① 確認の明細の購入日の前後だけ |
...
- **Zaim明細と照合するのは ① 確認の明細だけ。** 登録済みの明細は、Zaimにある自分自身と一致してしまう
