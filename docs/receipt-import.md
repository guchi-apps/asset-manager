...
### 「反映待ち」口座は登録先に選ばせない（Issue #443）

「反映待ち」口座へ登録した明細は**置き換え候補にならない**（後述の検証A）。それでも登録先の選択肢に
出ていたため、実際にこの口座へ登録された明細があった。いまは次の3か所で止めている。

- 登録先の選択肢（一覧の既定カード・詳細画面）から外す（`ReceiptFeatureStatus.pendingAccountIds`）
- 「正しい（登録）」の事前判定（`registerBlocker` の `cardIsPending`）
- `sendReceiptToZaim`（1件も送っていない明細だけ。途中まで送った明細は同じカードで続けるしかないため止めない）

**反映待ち口座は `ZAIM_PENDING_ACCOUNT_ID` だけでなく、口座名「反映待ち」でも見分ける**（`isPendingAccount`）。
この値は検証スクリプト用で、本番では設定していないことがあるため。すでにこの口座へ登録済みの
「反映待ち」の明細には、Zaimで出金元を請求元のカードへ変えるよう警告を出す。
...
- **「反映待ち」は手順の名前で、Zaimの「反映待ち」口座ではない。** 登録先は#302以降どおり請求元の
  カード。反映待ち口座へ入れた明細は置き換え候補にならない（後述の検証）ので、#443 から登録先に選べない
...
- 重複の可能性が残った明細は、「正しい（登録）」で確認ダイアログを挟む。「重複ではないので登録」は
