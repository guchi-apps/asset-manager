...
外してある**。`getReceiptOverviewAction()` は既定で `status: { not: "REPLACED" }` で引き、
一覧の末尾に総件数（`replacedCount`）のボタンだけを出す。押されたときに
`getReceiptOverviewAction(true)` で読み直し、**直近30件**（`REPLACED_TAKE`）を並べる。

- **取得上限100件は置き換え済みを含めない。** 以前は全状態から新しい順に100件だったため、
  置き換え済みが増えると古い「確認待ち」が一覧から押し出されていた
