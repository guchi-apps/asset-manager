...
- **AIDEが読んだ月の分しか無い。** 巡回ジョブ（`zaim-money-sync`）は当月だけを読んでおり、遡る日数が
  月をまたぐルールでも、先月ぶんの連携明細は入ってこない。当月＋先月を読み、読んだ月を `months` で
  返す変更は guchi-apps/aide#286（`lib/zaim-aide-money.ts` は `months` を読める。無ければ `null`）
