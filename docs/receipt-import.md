...
- **組にするのは、金額が概算の明細（下記）か、同じ口座の明細どうしに限る**（計画レビューの指摘）。組にした明細は
  「Zaimにだけ」「アプリにだけ」から消えるため、条件を絞らないと無関係な近い金額の2件が組になり、取り込み漏れ・未着が埋もれる
- 並べ方は 概算 → 同じ口座 → 差の割合が小さい → 日付が近い、の順に貪欲に確定させる。置き換え済みは相手にしない
