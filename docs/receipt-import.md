...
**「反映待ち」口座へ、品目付きで登録する**。統合そのものはZaim標準の
「置き換え」（スマートフォンアプリ限定）に任せる。

**登録の経路はIssue #302で変わった。** 以前はZaim APIで登録していたが、APIで作った明細は
置き換え候補にならないことが #300 の実測で分かったため、**AIDE経由でZaim Web版
（my.zaim.net）の入力画面へ登録する**形にしてある（後述「Zaimへの登録」「置き換えの成立条件と検証」）。
登録先の口座は「反映待ち」→（#302・#443）請求元のカード→（#464）「反映待ち」、と変遷している
（後述「登録先は『反映待ち』口座に固定する」）。

スマートレシート・Amazon由来の明細（#222）も、画像が無いだけで同じ流れに載せている。

画面は `/receipts`（**家計簿連携**）と `/receipts/<id>`（確認・修正）。スマホでの利用を主用途にしている。
`/receipts` は「明細 / 突合せ / 内訳 / 設定」の4タブ（突合せは #456。「内訳」は #466 で「内訳の提案」から
改名した）。内訳の自動振り分け・口座間コピーはすべて Issue #271 で足したもの（後述）。

Zaimを読む3つのボタンは、**どれも定期実行しない**（押したときだけ動く。毎晩のZaim自動取得は
評価額用の別処理で、`docs/zaim-auto-sync.md`）。置き場所は使う頻度で分けている（#452）。

| ボタン | 置き場所 | 何を読むか | 要るとき |
|---|---|---|---|
| Zaimのマスタを更新 | 設定タブ「連携の状態」（未取得のあいだだけ明細タブにも出す） | カテゴリ・内訳・口座の一覧（`syncZaimMasters`） | Zaimで内訳・口座を増減・非表示にしたとき |
| Zaim連携明細を取り込む | 明細タブの上 | スマートレシート・Amazonの直近60日（`importLinkedReceipts`。Web版はAIDEの1日2回の巡回結果） | 明細を確認するたび |
| Zaimから読み込む | 内訳タブ | 直近60日の支出（`refreshGenreSuggestions`）。全件を「すべての明細」に出し、内訳が未設定のものを提案にする。Zaimは変えない | 内訳タブを使うたび |

**マスタが0件のうちは取り込みが押せない**（連携口座を口座マスタから割り出すため）。明細タブから
マスタ更新を完全に外すと、初回に何をすればよいか分からなくなるので、`genreCount === 0` の間だけ残している。
      ...
      └ Claude API で構造化抽出（店舗・日時・商品・数量・単価・値引き・税・総額・信頼度）
          └ 商品分類履歴（ProductClassificationRule）でAIの分類を上書き
              └ 検算（明細合計 vs 総額）→ 高信頼なら CONFIRMED、それ以外は REVIEW_REQUIRED
                  └ 画面で確認・修正 → 確定（触った分類を履歴へ保存）→ CONFIRMED
                      └ カードの連携明細がZaimに届くのを待つ（#466。届いたかはAIDEが読んだWeb版の一覧で見る）
                      └ AIDE経由でZaim Web版へ商品ごとに登録（出金元＝反映待ち口座）→ SENT_TO_ZAIM
...
| `ZAIM_CARD_ACCOUNT_ID` | Gmail等の自動取込（`lib/payment-import.ts`）の既定の請求元カード `account_id`。レシートの登録先には使わない（#464） |
| `ZAIM_PENDING_ACCOUNT_ID` | **レシートの登録先の`account_id`（#464）。** 未設定でも口座名「反映待ち」で見分けられる |
...
# 3. 表示された2行を .env へ貼る
# 4. 「反映待ち」口座の id を調べて ZAIM_PENDING_ACCOUNT_ID に設定する（未設定でも口座名で見分けるが、
#    確実にしたい場合はここで設定する）。Gmail等の自動取込の既定カードを使うなら ZAIM_CARD_ACCOUNT_ID も
...
- **出金元は「反映待ち」口座に固定する（Issue #464）。** 画面でカードを選ぶ操作は無くした。
  口座は `ZAIM_PENDING_ACCOUNT_ID`、無ければ口座名「反映待ち」で口座マスタから見つける
  （`isPendingAccount`）。見つけた口座idは `ReceiptImport.zaimAccountId` に残す
...
| 状態 | 意味 |
| --- | --- |
| `CONFIRMED` | 確定済み。反映待ち口座へ登録できる（画面の手順は「② 反映待ち」。#466） |
| `SENT_TO_ZAIM` | 反映待ち口座へ登録済み。Zaimアプリでの置き換え待ち（画面の手順は「③ 反映」。#466） |
...
- 代わりに、`SENT_TO_ZAIM` のレシートを**「反映待ち口座へ登録済み・置き換え待ち」として並べ、
  Zaimアプリで的を探すのに要る値（日付・金額・店舗）を出す**だけにした
- 置き換えが済んだら、画面の「置き換えた」で `REPLACED` を記録する（記録した明細は一覧から外れる。#456）

### 置き換える相手の候補を出す（Issue #443）

置き換え前のカード連携明細は**公開APIからは見えないが、AIDEが巡回したZaim Web版の一覧
（#383。後述「AIDE経由でWeb版の明細を合流させる」）には出る**。そこで「反映待ち」の明細に、
この一覧から選んだ候補を出している（`lib/replace-target.ts` の `findReplaceTargets`、
`lookupReplaceTargets` → `getReplaceTargetsAction`）。

- 条件は**金額が総額と一致**かつ**日付が購入日の前後3日以内**（`REPLACE_TARGET_WINDOW_DAYS`）。
  **口座では絞らない**（登録先の食い違いに気づけるよう、候補には「登録したカードと違う口座」と出す）
- **`loadWebMoneyEntries` を通さず、`fetchZaimMoneyListFromAide` の生の一覧から選ぶ**（計画レビューの指摘）。
  合流側は口座名をマスタで引けない行を落とすため、通すとカードがマスタに無い環境で黙って0件になる
- 外すのは振替の行と、当アプリが登録した行（コメント `Asset Manager レシート取込 #`）だけ
- **候補があっても置き換えが済んでいないとは限らない。** 置き換え済みの元明細も一覧に残るため
  （後述「内訳の提案」）、状態は自動で進めない
- 見つからないときは、購入日の前後が**AIDEの読んだ月**に収まっているかで文言を分ける（`notCovered` / `notFound`）。
  読んだ月は応答の `months` から取り、無い（当月だけを読んでいた）AIDEでは巡回時刻の月とみなす
  （`resolveCoveredMonths`）。先月分の巡回は guchi-apps/aide#286 で対応する
- 一覧は画面の表示を止めないよう、明細・一覧を出したあとに読む（`useReplaceTargets`）
- **Issue #451 で、対象を「確認」ステップ（`ANALYZING`/`REVIEW_REQUIRED`/`CONFIRMED`）にも広げた。**
  条件・データソースは変えず、`lookupReplaceTargets` の対象状態に確認ステップを加えただけ。
  まだ登録前の明細に「Zaimへ既に反映されていそうな候補」を示す用途で、バッジは共通のまま、
  パネルの文言だけ `step`（`"review"` / `"waiting"`）で出し分けている
  （`ReplaceTargetBadge` / `ReplaceTargetsPanel`）

### 登録するとき、購入日をZaimの連携明細の日付へ合わせる（Issue #455）

Gmailのカード利用通知などから取り込んだ購入日と、Zaimへ届いたカード連携明細の日付は数日ずれることがある。
`sendReceiptToZaim` は登録の直前に同じ一覧から相手を探し（条件は上と同じ「金額一致・前後3日以内」）、
**登録先と同じカードの明細がちょうど1件**なら、その日付でZaimへ登録し、Asset Manager側の購入日も
その日付へ書き換える（`pickAlignedPurchaseDate`）。画面の単件登録・まとめて登録・詳細画面のほか、
ChatGPT/AIDEからの取り込み（`lib/payment-import.ts` の `importPayment`）の自動登録にも効く。

- **合わせないとき**は元の購入日のまま登録する: 候補が0件・2件以上（同額の買い物や置き換え済みの元明細との
  取り違えを避ける）、同じカードの明細が無い、カードが口座マスタに無い、AIDEを読めない・未巡回
- **途中まで送った明細（`MANUAL_ACTION_REQUIRED` の再送）は合わせない。** 登録済みの商品と日付が食い違うため
- **③ 反映（登録済み）の明細は合わせない。** Zaimへ登録済みの明細の日付を書き換える口がAIDEに無いため。
  カード連携明細はカード利用通知より遅れて届くことが多く、**登録の時点で相手がまだ無ければ合わせられない**（仕様）
- 購入日は登録より先に書き換える（途中で止まっても、登録した日付と画面の購入日が揃う）。書き換えた購入日は
  時刻を持たない日付（JST 00:00）になり、通知由来の時刻は消える
- まとめて登録では一覧を1回だけ読む（`SendReceiptOptions.loadAideMoneyList`）

### 登録先は「反映待ち」口座に固定する（Issue #464。#443からの訂正）

以前は逆に、「反映待ち」口座への登録はZaimの置き換え候補にならないという理解のもと、登録先の
選択肢から外し、カードを選ばせていた（#443。根拠は下の検証A・B）。**#464で実機のZaimアプリで
確認したところ、反映待ち口座への登録も置き換え候補になることが分かった。** カードを選ばせる意味が
無くなったため、選択自体を廃止し、登録先を「反映待ち」口座に固定した。

検証A・Bの再解釈: どちらもZaim APIで作った明細で、出金元を反映待ち口座にしたA・連携カードに
しただけのB のどちらも候補に出ておらず、**出金元の違い（A・B間）は結果を左右していなかった**。
「反映待ち口座は」置き換え候補にならない、と言えるだけの比較にはなっていなかったことになる。
分かれ目は「Zaim APIで作ったか、Web版で作ったか」で、これは今回の変更でも変わらない
（登録経路はWeb版のまま。前述「なぜ支出の登録もAIDE経由になったのか」）。

「反映待ち」口座は `ZAIM_PENDING_ACCOUNT_ID` だけでなく、口座名「反映待ち」でも見分ける
（`isPendingAccount`）。この口座が口座マスタから見つからない場合は、「登録」を`registerBlocker`が
止める（`PENDING_ACCOUNT_UNAVAILABLE_MESSAGE`）。

### 口座の種別（Issue #471）

**置き換えできるのはカード・電子マネーの連携明細だけ**で、銀行口座・デビットカード・ショッピングサイトの
連携明細は対象外（後述「置き換えの成立条件と検証」）。銀行口座の連携明細に届いたGmail等の明細を
反映待ち口座へ登録すると、置き換えられないまま同じ支払いが二重に残る。そこで口座マスタ（`ZaimAccount`）に
種別（`kind`）を持たせ、設定タブの「口座の種別」で確かめ・選び直せるようにした。

| 種別 | 意味 | ② 反映待ちでの扱い |
| --- | --- | --- |
| `CARD` カード・電子マネー | 連携明細を置き換えられる | 従来どおり「Zaimへ登録」 |
| `BANK` 銀行・デビット | 連携明細を置き換えられない | 「連携明細で済ませる」（登録せず `REPLACED`）。「それでも登録」も残す。まとめて登録から外す |
| `MANUAL` 手入力 | 現金・財布など。連携明細は届かない | この口座の行は連携明細の候補にしない |
| `PENDING` 反映待ち | 登録先（#464）。口座名で決まり、選び直せない | 同上 |
| `OTHER` 対象外 | 証券・ポイント・Amazon等の連携サービス | 従来どおり |
| `null` 未設定 | 分からない | **カード扱い**（#471 より前と同じ動き） |

- **Zaim APIの口座マスタは種類を返さない**（`id`・`name`・`sort`・`active` だけ）。そのため
  「Zaimのマスタを更新」（`syncZaimMasters`）で**口座名の語**と**AIDEの残高一覧**（連携しているか＝
  `lastUpdatedAt` の有無、残高の符号＝カードはマイナス）から推定する（`lib/zaim-account-kind.ts` の
  `guessAccountKind`）。名前の語を優先し、「デビットカード」はカードより先に銀行・デビットへ寄せる
- 人が選んだ種別は `kindManual` を立て、マスタを取り直しても推定で上書きしない（内訳の `hidden` と同じ考え方）
- 手入力・反映待ちの口座の行は、置き換え候補（`findReplaceTargets`）にも突合せ（`reconcileReceipts`）にも
  出さない。Web版の一覧には人やアプリが入れた明細も並ぶが、それは置き換える側の明細で、連携明細ではない
- 突合せの「Zaimにだけある」は、登録先・既定のカードに加えて**種別がカード・電子マネーの口座**も対象にする
- 「連携明細で済ませる」（`settleWithLinkedEntry`）は `CONFIRMED` だけを `REPLACED` にする。状態を増やさないのは、
  「Zaim側の記録で済んだ」「画面に出さない」「二重取り込みの判定には含める」がどれも「置き換えた」と同じため

### 置き換え済みは画面に出さない（Issue #378・#456）

**家計簿連携は家計簿の記録を残すための機能ではない。** Zaimへ記録するときにGmailなどの明細を
使うための機能なので、置き換え済み（`REPLACED`）は画面のどこにも並べない（#456）。
#378 では件数のボタンと直近30件の一覧を残していたが、#456 で #431 の「③ 反映済み」ごと外した。

- **取得上限100件は置き換え済みを含めない**（#378）。以前は全状態から新しい順に100件だったため、
  置き換え済みが増えると古い「確認待ち」が一覧から押し出されていた（実測: 置き換え済み102件で
  それより古い「確認待ち」は一覧から消えた）
- DBの状態としては残す。「置き換えた」を押した明細が次の取り込みで復活しないのは、取り込み側が
  置き換え済みも含めて二重取り込みを判定しているため（後述「二重取り込みの防止」）
- 詳細画面（`/receipts/<id>`）は変えていないので、置き換え済みもURLからは今までどおり開ける

**機械が置き換えの成否を判定することはできない。** 置き換えの操作はスマートフォンアプリ限定で、
置き換え前の明細はAPIから見えない（Web版の一覧には出るが、置き換え済みの元明細も残る）。
置き換え後にどの明細が何に変わったかも追えない。
自動削除・自動統合も行わない。

## 明細タブの手順（Issue #431・#466）

「明細」タブは、**「① 確認 → ② 反映待ち → ③ 反映」**の手順で並べている。
DBの状態（`ReceiptStatus`）は増やしておらず、対応は `lib/receipt-flow.ts` の
`receiptFlowStep` だけで決める（一覧・詳細画面・突合せ・テストが同じ対応を見る）。

| 手順 | 状態 | 行で押す操作 |
| --- | --- | --- |
| ① 確認 | `REVIEW_REQUIRED` / `ANALYZING` | 「確定」「修正」「削除」 |
| ② 反映待ち | `CONFIRMED` | 「Zaimへ登録」（連携明細がまだなら「待たずに登録」）「修正」「削除」 |
| ③ 反映 | `SENT_TO_ZAIM` | 「置き換えた」（押すと一覧から外れる） |
| 画面に出さない | `REPLACED` | なし（#456） |
| 手順の外（赤帯） | `MANUAL_ACTION_REQUIRED` / `FAILED` | 詳細画面で対応する |

**#466 で手順の意味が変わった。** Gmail等の明細は早く届き、カードの連携明細は利用から数日遅れてZaimに届く。
#466 より前は「確認」で「登録」を押した時点でZaimへ送っており（`confirmAndSendReceipt`）、この待ち時間が
手順に表れていなかった。いまは **確認で中身を確定 → 連携明細の到着を待つ → 届いたらZaimへ登録して置き換える**。

| | #431〜#465 | #466 以降 |
| --- | --- | --- |
| `CONFIRMED` | ① 確認（「確認済み・未登録」） | **② 反映待ち**（「確定済み」） |
| `SENT_TO_ZAIM` | ② 反映待ち | **③ 反映**（「Zaim登録済み」） |

**古いIssue・コメント・この文書の古い節で「反映待ち（の明細）」と書かれているものは、いまの「③ 反映」にあたる。**
コード上の名前も `waiting` が指す状態が変わっているので、手順名で分岐を書くときは `receiptFlowStep` の
戻り値（`review` / `waiting` / `reflect`）を使い、状態を直接比べない。「まだZaimへ登録していないか」は
`isBeforeZaimRegister` で判定する（削除できる・Zaimの明細との重複を見る、はこちら）。

- **「反映待ち」は手順の名前で、Zaimの「反映待ち」口座とは別のもの。** 口座は登録先（#464）で、
  手順は「連携明細の到着待ち」。名前が同じなので、文書では口座のほうを必ず「反映待ち**口座**」と書く
- **「確定」はZaimへ送らない**（`confirmReceiptAction`）。押せるかは `confirmBlocker` が判定し、条件は
  `confirmReceipt` が弾くもの（検算・内訳・購入日）に揃えてある。店舗名・反映待ち口座・AIDE設定は見ない
- **「Zaimへ登録」は `sendReceiptToZaimAction`。** 押せるかは `registerBlocker` が判定する（上に加えて
  店舗名・反映待ち口座の有無・AIDE設定）。**どちらかの関数の条件を変えたら、対応するblockerも直す**
- **AIが高信頼と判定した明細は取り込み時点で `CONFIRMED` になるため、①を通らず最初から②に並ぶ。**
  ②からも「修正」「削除」はできる。詳細画面で②の明細を保存すると検算し直して①へ戻ることがあるため
  （`saveReceipt`）、詳細画面の「Zaimへ登録」だけは保存 → 確定 → 登録を続けて行う（`confirmAndSendReceipt`）
- ②の行には、**見つかった連携明細の中身**（日付・店舗・金額・口座）を出す（`FoundLinkedEntries`）。
  探し方は置き換え候補（#443）と同じで、登録前の明細なのでidを指定して読む（`useReplaceTargets(ids)`。
  `"all"` は登録済みの明細しか返さない）
- 「連携明細が届いたN件をZaimへ登録」は、②のうち連携明細が見つかり、`registerBlocker` に掛からず、
  重複の可能性も無い明細だけを送る。**送る明細は `onlyReceiptIds` でidを指定する。** 送らない明細を
  `skipReceiptIds` で数え上げる形にすると、一覧（100件まで）に出ていない確定済みの明細が漏れて送られてしまう
- 連携明細がまだ無い行は「待たずに登録」になり、確認を挟む（`RegisterWithoutLinkDialog`）。AIDEが一覧を
  読めないときや、連携しないカードで払った明細は、待っていても届かないため
- 反映したかどうかは機械では判定しない（前述「置き換え待ち」）。③には置き換えの手がかりと、
  登録からの経過日数（JSTの暦日。`WAITING_STALE_DAYS` 以上で色を変える）、
  Web版の一覧に候補があるか（#443。「連携明細あり / 連携明細はまだ / 確認できない」）を出す。
  ②の経過日数は確定した時刻を持っていないので、取り込み（`createdAt`）から数える

## 突合せタブ（Issue #456・#466）

**「アプリの明細」「Zaimの明細」「突き合わせ」の3画面**に分けている（#466）。突き合わせが組にした結果で、
前の2つはその材料になっている両側の一覧。**3つとも同じ1回の読み込み（`getReconciliationAction`）から作る**
ので、画面を切り替えてもZaim・AIDEへの問い合わせは増えない。

- アプリの明細: 手順に載っている明細（`ReceiptSummary`）を全部出し、組の結果から「連携明細あり /
  連携明細はまだ / 未判定」を添える。**組（`pairs`）から作らず一覧から作る**のは、購入日・金額が無いなどで
  判定できなかった明細（`uncheckedCount`）が組に入らず、消えてしまうため
- Zaimの明細: 組のうちZaim側がある行（一致・Zaimにだけ）を日付の新しい順に出し、「アプリに明細あり /
  アプリに無い」を添える。「Zaimにだけ」と同じく、突き合わせる口座の行だけが並ぶ（後述）

「突き合わせ」は、**Zaimのカード連携明細**（AIDEが巡回したWeb版の一覧）と、① 確認・② 反映待ち・③ 反映の
明細を1対1の組にして「一致 / Zaimにだけある / アプリにだけある」に分けて並べる。
Zaimにだけある明細は「Gmailなどの取り込み待ちか、Zaimアプリで直接入力する」もの、
アプリにだけある明細は「カードの明細がまだZaimへ届いていない」ものの手がかりになる。
組を作るのは `lib/receipt-reconcile.ts` の `reconcileReceipts`（純粋関数）、
読み出しは `lookupReconciliation` → `getReconciliationAction`（タブを開いたときに読む）。

- **照合の条件は置き換え候補（#443）と同じ**（金額一致・日付が前後3日以内、同じ口座 → 日付の近い順）。
  候補の組をすべて作ってから貪欲に確定させ、1件のZaim明細を2件の明細へ割り当てない
- **組を作るときは口座で絞らない**（#443 と同じ。口座名の表記揺れやマスタの欠けで黙って「一致」が0件になり、
  一覧の「連携明細あり」バッジと食い違うため）。口座は並び順と「登録したカードと違う口座」の印にだけ使う
- **「Zaimにだけある」は「突き合わせる口座」の行だけに絞る。** 明細の登録先カードと既定のカード
  （`ZAIM_CARD_ACCOUNT_ID`）で、「反映待ち」口座は含めない。Web版の一覧には銀行・現金・手入力の
  明細も並び、**置き換え待ちかどうかを読めない**（前述「置き換え待ち」）ため、絞らないと関係の無い明細で埋まる
- **「反映待ち」口座は登録先の実カードではないので、`cardAccountName`（＝上の「口座」）には
  含めない**（Issue #464。計画レビュー指摘）。含めると、反映待ち登録した明細の`sameAccount`が
  常にfalseになり、「登録したカードと違う口座」がすべての明細に誤って出る
- **置き換え済み（`REPLACED`）の明細も照合の相手に読む**（画面には出さない）。Web版の一覧には置き換え済みの
  元明細も残るため、読まないと済ませたカード明細が全部「Zaimにだけある」に並ぶ。置き換え済みと組になった
  Zaim明細は結果から外す。期間の手前の置き換え済みも、日付のずれ（3日）の分だけ読む。
  それでも「置き換えた」を押していない行は混ざりうるので、画面で断っている
- **Zaimへ登録する前（① 確認・② 反映待ち）の明細で「重複の可能性」（#445）に出たZaim明細とは組にしない**（#451 の `excludeDismissedAsDuplicate` と同じ規則）。
  同じZaim明細に「重複（消すべき二重の記録）」と「一致（登録してよい）」の逆の印が付くのを避けるため。
  重複の結果は画面が読んでいるので、突合せはその読み込みを待ってから相手のidを渡して読む。
  その明細は手入力の明細なので「Zaimにだけある」にも出さない
- 振替、当アプリが登録した行（`Asset Manager レシート取込 #`）、口座間コピーの行（`Asset Manager 複製 #`）は外す
- 期間は直近31日（`RECONCILE_LOOKBACK_DAYS`）。**前後3日がAIDEの読んだ月に入らない明細は
  「アプリにだけ」と言い切れないので出さず**、件数だけ示す（`uncheckedCount`）。ただし今日より後の日付は
  まだZaimに明細があり得ないので、その月を読んでいなくても判定する（月末の明細が判定不能にならないように）
- 一致した明細が反映待ちなら、その場で「置き換えた」を押せる。確認中なら詳細画面へ誘導する

### 銀行・デビットの連携明細は「置き換え不可」として出す（Issue #514）

**組を作るときに銀行・デビット（`BANK`）の明細を外してはいない。** 金額・日付が一致していれば
「一致」として出すが、Zaimの置き換えの対象外（前述「口座の種別」）なので、**そのまま「開いて登録」へ
誘うと二重に残る**。そこで `ReconcileEntry` に `accountKind` を持たせ、画面（`components/receipts/reconcile-view.tsx`）
が次のように出し分ける。判定は明細タブ・詳細画面と同じ `isReplaceableKind`（`lib/zaim-account-kind.ts`）。

- 組の枠を橙にし、「置き換え不可（銀行・デビット）」の印と、二重に残る旨の一文を出す
- 「開いて登録」は出さず、「メモを書く」と「連携明細で済ませる」（#471 の `settleWithLinkedEntry`）を出す
- 絞り込みの下に「『一致』N件のうちM件は銀行・デビットで置き換えできません」を出す
- 「アプリの明細」「Zaimの明細」の行にも同じ印を付ける。「Zaimの明細」の該当行は**明細へのリンクにしない**
  （押して開くと、登録して置き換える導線に見えるため）

**「Zaimにだけある」には出ない。** 突き合わせる口座（`accountNames`）はカード種別の口座と明細の登録先だけで、
銀行・デビットの口座は入らないため（上の「『Zaimにだけある』は…」と同じ絞り）。

### 置き換えできない連携明細のメモを直接書き換える（Issue #514）

置き換えられない以上、詳細を残せる場所は**その連携明細のメモ**しかない。突合せタブの「メモを書く」から、
アプリの明細の品目で作った下書き（`lib/zaim-memo-draft.ts` の `buildZaimMemoDraft`）を直して書き込む。

- **上限は100文字**（Zaimの制限。AIDE側 `write.ts` の `MAX_TEXT_LENGTH` と同じ）。入りきらない品目は
  「ほかN件」に畳み、**その分も含めて**上限に収める
- **日付・金額は画面から受け取らない。** AIDEが読んだ一覧（`fetchZaimMoneyListFromAide`）から引き直す
  （`writeZaimEntryMemo`）。AIDE側は編集画面を開いたときの日付・金額が本文と一致しなければ取り違えとして
  止めるので、その照合材料を画面から渡すと検知が形だけになる。一覧に無いidは書き換えない
- **置き換えできる口座（カード等）の明細は断る。** そちらは置き換えで反映する経路に載っている
- **冪等キーは本文を混ぜる**（`buildZaimMemoRequestId`）。明細idだけで作ると、書き直したメモをAIDEが
  「処理済み」として黙って捨てる。同じ本文の押し直しだけが `duplicated` になる
- **書き込んだメモはAIDEの次の巡回（1日2回）まで一覧に出てこない。** 画面側で書いた本文に差し替えて見せる

**AIDE側の受け口（`POST /api/zaim/payment/web/memo`）はまだ無い**（guchi-apps/aide#354）。
受け口が出るまで404 → `notImplemented` で止まり、ダイアログの「コピー」からZaimアプリへ貼り付ける。
`lib/zaim-web-genre.ts` が aide#273 のデプロイを待っていたのと同じ形で、失敗の分類もそちらに揃えている。

### 金額ずれ（Issue #483）

Gmailのドル建て請求を円に換算して取り込むと、カード会社の換算レート・手数料のぶん、Zaimに届くカード連携明細と
数円〜数十円ずれる。完全一致しか見ないと、同じ支払いが「Zaimにだけ」「アプリにだけ」に分かれてしまう。
そこで**金額が一致した組を確定させたあと、残りから近い金額どうしを「金額ずれ」の組にする**（`reconcileReceipts`）。

- 条件は日付が前後3日以内、差が `AMOUNT_GAP_RATIO`（Zaim側の5%）と `AMOUNT_GAP_MIN_YEN`（50円）の大きいほう以内（`isNearAmount`）
- **組にするのは、金額が概算の明細（下記）か、同じ口座か、店舗名が似ている明細どうしに限る**（計画レビューの指摘）。
  組にした明細は「Zaimにだけ」「アプリにだけ」から消えるため、条件を絞らないと無関係な近い金額の2件が組になり、取り込み漏れ・未着が埋もれる
- **店舗名が似ているとは、`isSameStore`（`lib/receipt-duplicates.ts`）で、明細の店舗名がZaim側の加盟店名（`place`）か品名（`name`）と
  片方が他方を含むこと**（実例: #487）。Gmailの取り込みに概算の印が付かず、既定のカード（`ZAIM_CARD_ACCOUNT_ID`）が
  Zaimの口座と違うと、概算・同じ口座のどちらも満たせない。「Anthropic ¥17,500」と「ANTHROPIC* CLAU… ¥17,644」（差0.8%）が
  組にならず「アプリにだけ」「Zaimにだけ」へ分かれていた
- 並べ方は 概算 → 同じ口座 → 店舗名が似ている → 差の割合が小さい → 日付が近い、の順に貪欲に確定させる。置き換え済みは相手にしない
- 「重複の可能性」に出たZaim明細とは、一致と同じく組にしない

**Zaimの金額を正とする。** 「Zaimの金額に合わせる」（`alignReceiptAmountToZaim`）は、アプリ側の総額と商品の金額だけを
書き換え、**Zaimへは何も送らない**。

- 押せるのはZaimへ登録する前（① 確認・② 反映待ち）で、**商品が1件の明細だけ**。複数の商品はどれの金額を変えるか決められない
- ③ 反映（登録済み）はZaimに誤った金額で入っているので、アプリ側だけ直しても置き換えられない。組には出すがボタンは出さない
- 画面に出ていた金額（`expectedAmount`）と今の金額が違えば止める。差が `isNearAmount` の範囲を超える書き換えも止める
- 元の金額は `ReceiptImport.amountAdjustedFrom`（2回合わせても最初の金額）に残し、概算の印（`amountApproximate`）は外す

### 概算の印（Issue #483）

取り込みAPI（`POST /api/receipts/import`）は任意で金額の精度を受け取る。

| 項目 | 意味 |
|---|---|
| `amountApproximate` | 金額が正確でない可能性がある（true/false） |
| `amountNote` | 理由（191文字まで）。例: 「USD 9.99 を 1ドル=150.2円で換算」 |
| `originalAmount` / `originalCurrency` | 外貨建ての元の金額と通貨（ISO 4217の3文字） |

- `amountApproximate` を省いても、`originalCurrency` がJPY以外なら概算として扱う（`isApproximateAmount`）。理由が無ければ
  「USD 9.99 を円に換算した金額」を入れる（`describeAmountNote`）
- **概算の取り込みは、分類履歴に一致してもZaimへ自動登録しない。** 確定まで進めて② 反映待ちで止め、`pendingReview` を返す。
  誤った金額で登録すると、カードの連携明細と金額が合わず置き換えられないため
- 詳細画面で人が総額を書き換えたときも、その金額を確かめたものとして概算の印を外す（`updateReceipt`）
- **送り手（AIDEの `asset_manager_import_payment`・ChatGPTスケジュール）が新しい項目を送るかはAIDE側の入力スキーマ次第。**
  送られてこない間は印は付かないが、「金額ずれ」の組（同じ口座どうし）と金額合わせはそれでも使える

## 重複の可能性を出す（Issue #445）

同じ支払いが、Gmailのカード利用通知・car-careの給油記録・スマートレシート・Amazon・
Zaimアプリでの手入力と、複数の経路から記録されることがある。「明細」タブの手順に載っている
明細（① 確認・② 反映待ち・③ 反映）に、同じ支払いと思われる相手を並べる。**消すかどうかは人が決める**（自動削除・自動統合はしない）。
判定は `lib/receipt-duplicates.ts`（純粋関数）、DBとZaim APIの読み出しは `lookupReceiptDuplicates`。

| 相手 | 読み方 |
| --- | --- |
| 取り込み同士（置き換え済みを含む） | DB |
| Zaimに記録済みの明細（手入力・置き換え済み） | Zaim公式APIの `GET /v2/home/money`。Zaimへ登録する前（① 確認・② 反映待ち）の明細の購入日の前後だけ |

- **判定は「金額が一致」かつ「同じ日なら店舗名を問わない／前後3日以内なら店舗名も一致」。**
  カード利用通知の加盟店名（「ENEOS」）と給油記録の店舗名（「エネオス 西新井店」）は揃わないため、
  同じ日は店舗名を見ない。日がずれるときまで店舗名を見ないと、別の買い物を拾いすぎる。
  日付の幅は置き換え候補（#443）の `REPLACE_TARGET_WINDOW_DAYS` をそのまま使う
- **Zaim側の相手はWeb版の一覧（AIDE）から取らない。** Web版にはカードの自動連携明細も並ぶため、
  置き換える相手（#443）まで重複として出てしまう。公式APIが返すのは手入力と置き換え済みの明細だけ
  （前述「置き換え前のカード連携明細は、公開APIから見えない」）なので、こちらを使う。
  #443 でアプリから「反映待ち」口座へ登録できなくなったため、拾う相手は主に**Zaimアプリで手入力した明細**になる
- **Zaim明細と照合するのは、Zaimへ登録する前（① 確認・② 反映待ち。`isBeforeZaimRegister`）の明細だけ。** 登録済みの明細は、Zaimにある自分自身と一致してしまう
- 当アプリが登録した明細（コメントの印 `OWN_REGISTRATION_COMMENT_PREFIX`）と、取り込み元のZaim明細
  （`ReceiptItem.sourceZaimMoneyId`）は相手にしない。どちらも取り込み同士の照合で見ている
- 置き換え済みの明細は商品ごとに分かれるため、同じ日・同じ口座・同じ店の明細を合算した金額でも照合する
- Zaimを読めない（未設定・通信失敗）ときも、取り込み同士の照合は返す。理由は画面のお知らせに出す
- 一覧・詳細の表示を待たせないよう、置き換え候補（#443）と同じく表示のあとに
  `getReceiptDuplicatesAction` で読む。**画面を開くたびにZaim APIへの読み取りが1回増える**

### 「重複ではない」と、登録前の確認

- 「重複ではない」を押した組は `ExternalPaymentImport`（`source: "receipt-duplicate-dismissed"`、
  `externalId` に組のキー）へ残し、次から出さない。削除した連携明細の記録（#431）と同じく、テーブルを
  増やさないための流用で、`source` が違うのでGmail・car-careの重複判定とは交わらない。
  取り込み同士の組のキーは `receipt:<小さいid>-<大きいid>` で、どちらの明細から押しても同じになる
- 明細を削除すると、その明細で押した記録も消す
- 重複の可能性が残った明細は、「登録」で確認ダイアログを挟む。「重複ではないので登録」は
  候補をすべて「重複ではない」と記録してから登録する
...
「反映待ち」口座へ登録する。画像が無いだけで、確認・確定・登録の流れは撮影したレシートと同じ。
...
- **保存（`updateReceipt`）でも印を落とさない。** 保存は商品行を全部消して作り直すため、以前は
  `sourceZaimMoneyId` が `null` になり、一度でも修正・確定した連携明細は次の取り込みで同じ
  `sourceKey` の取り込みへ商品行が足し直されていた（#431の計画レビューで判明。削除機能と無関係に
  起きる経路）。作り直す行へ元の行の値を引き継ぎ、**保存で外した行の元明細idは削除と同じく記録する**

### 触らないもの

**Zaim側の元明細には一切手を触れない。** 削除も、集計対象外への変更もしない。
反映待ち口座へ登録したあと、元のスマートレシート・Amazon明細を集計対象外にする操作は
これまでどおりZaimの画面で行う（Zaim APIの支出更新に集計対象外を切り替える項目が無く、
自動削除・自動統合を行わないという #153 の方針にも合わせている）。

## 内訳の自動振り分け（Issue #271）

`/receipts` の「内訳」タブ（#466 で「内訳の提案」から改名）。**連携口座に限らずZaim全体の支出**から、
内訳が決まっていないものを集めて提案する。

タブの中は2画面（#466）。「すべての明細」は読み込んだ直近60日の支出を、内訳が決まっているものも含めて
全件出す（`toZaimPaymentRows`。集計対象外・金額0以下は出さない）。「未決定」がそこから拾った提案の画面で、
中身は従来どおり。**全件の一覧はDBに保存しない**（`SuggestionRefreshResult.payments` として読み込みの
結果に載せ、画面が持つだけ）。そのため「Zaimから読み込む」を押すまでは空で、画面を開き直すと消える。
提案（`ZaimGenreSuggestion`）はDBに残るので、「未決定」の件数は開いた時点から出る。実装は `lib/zaim-genre-suggest.ts`（判定）と `lib/kakeibo-service.ts`（DB・API）。
...
- **月が変わると、先月ぶんのWeb版由来の提案は消える（仕様）。** Web版の一覧は当月ぶんしか無く、
  読めた回は作り直すため。AIDEが先月分も読むようになると（guchi-apps/aide#286）、先月分の連携明細も
  提案に加わる。**提案はWeb版の明細を日付で絞っていない**（`refreshGenreSuggestions`）ので、分類履歴で
  決まらずAIへ渡す件数もおおよそ倍になる。先月分も提案の対象にすると決めたうえでの仕様（#443）
...
**巡回ジョブが動いていても、読めているのは画面に最初に描かれた約23行だけ**（#593。2026-09-25時点、
guchi-apps/aide#481 で対応待ち）。AIDEの `money-list.mjs` は一覧のDOM行を読むが、Zaim Web版の一覧は
仮想スクロールで、**表示中の行しかDOMに無い**（`page=` も効かない）。そのため直近2〜3日より前の明細は
1件も届かず、「スマートレシート → 反映待ち」の複製に9/7の明細が出てこなかった。

- 見分け方: 巡回ログの件数が「202608・202609分の明細 46 件」のように**月あたり23件前後で毎回同じ**なら、この状態
- 全件は、同じページが裏で読む `GET https://zaim.net/money/details?month=YYYYMM` のJSONにある
  （2026-09-25の実測で `202609` は209件。明細idも `id` で直接取れる）
- **Zaimの「月」は暦月ではない。** 月の開始日（この家計簿は25日）で区切られ、`month=202609` は
...
- **AIDEが読んだ月の分しか無い。** 巡回ジョブ（`zaim-money-sync`）は当月＋先月（JSTの暦月）を読み、
  読んだ月を `months` で返す（guchi-apps/aide#286。`lib/zaim-aide-money.ts` は `months` を読める。無ければ `null`）。
  **ただし2026-09-25時点では、各月とも一覧の表示行（約23行）しか取れていない**（前述、#593・aide#481）
...
**この経路は「反映待ち」口座（Issue #464）へは切り替えていない。** 画面（`/receipts`・
`/receipts/<id>`）からの登録とは別に、`lib/payment-import.ts` の `importPayment` が
`sendReceiptToZaim` を `{ fromAccountId: cardAccountId }` で直接呼び、`accountHint`（メールの
支払方法）か `ZAIM_CARD_ACCOUNT_ID` の既定カードへ登録する（下記「請求元カードの解決」）。
カードが1件も決まらないと `pendingReview`（確認待ち）になり、確認画面（`/receipts`）で登録すれば
そちらは反映待ち口座へ登録される。**画面からのカード選択をやめた効果はこの経路には及ばない**
（計画レビュー指摘。#464のスコープは画面からの登録のみ）。
