...
- 評価額の取得（`lib/zaim-scraper.ts` のPlaywright巡回）はこれまでどおり。支出の登録・参照だけAPIを使う
...
両者は共存する。Playwright側のstorage stateとセッション維持ジョブはそのまま残る。
...
（`lib/zaim-scraper.ts` の `toMatchKey` と同じ理由）。
...
Issueの Phase 5（スマートレシート由来明細の補正）と Phase 6（Amazon明細）は、
**Zaim上でそれらを識別できるかが未調査**のため今回の範囲に含めていない。
Zaim APIの `money` レスポンスには「どの連携サービス由来か」を示す項目が無く、
`place` や `name` の文字列から推測するしかない。実データを見て判断する必要がある。
