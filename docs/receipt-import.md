...
「明細」タブは、状態ごとのカードを縦に並べるのをやめ、**「① 確認 → ② 反映待ち → ③ 反映済み」**の
3手順で並べている。DBの状態（`ReceiptStatus`）は増やしておらず、対応は `lib/receipt-flow.ts` の
`receiptFlowStep` だけで決める（一覧・詳細画面・テストが同じ対応を見る）。

| 手順 | 状態 | 行で押す操作 |
| --- | --- | --- |
| ① 確認 | `REVIEW_REQUIRED` / `CONFIRMED` / `ANALYZING` | 「正しい（登録）」「修正」「違う（削除）」 |
| ② 反映待ち | `SENT_TO_ZAIM` | 「反映を確認した」 |
| ③ 反映済み | `REPLACED` | なし（開いたときだけ読む。#378） |
| 手順の外（赤帯） | `MANUAL_ACTION_REQUIRED` / `FAILED` | 詳細画面で対応する |

- **「反映待ち」は手順の名前で、Zaimの「反映待ち」口座ではない。** 登録先は#302以降どおり請求元の
  カード。反映待ち口座へ入れた明細は置き換え候補にならない（後述の検証）
- **「正しい（登録）」は確定とカード登録を1回で行う**（`confirmAndSendReceipt`）。判定は
  `confirmReceipt` と `sendReceiptToZaim` に任せて順に呼ぶだけ。**AIDEの登録設定が無いときは確定もしない**
  （状態だけ変わって何も登録されていない、に見えるため）。確定だけ済んで登録で止まった明細は
  「確認済み・未登録」のまま確認の手順に残る
- 押せるかどうかは `registerBlocker` が押す前に判定し、押せない理由を行に出す。条件は上の2関数が
  弾くもの（検算・内訳・購入日・店舗名・カード・AIDE設定）に揃えてあるので、**どちらかの条件を
  変えたらここも直す**
- 反映したかどうかは機械では判定しない（前述「置き換え待ち」）。代わりに置き換えの手がかりと、
  登録からの経過日数（JSTの暦日。`WAITING_STALE_DAYS` 以上で色を変える）を出す
...
- **画面で削除した取り込みの元明細も取り込まない（#431）。** 取り込み済みの印は
  `ReceiptItem.sourceZaimMoneyId` だけで、取り込みを削除すると商品行ごと消えるため、
  何も残さないと**次の「Zaim連携明細を取り込む」で同じ明細が戻ってくる**（#431で変更前のコードに
  対して再現した）。削除のときに元明細のidを `ExternalPaymentImport`（`source: "zaim-linked-deleted"`、
  `externalId` に明細id）へ残し、取り込み時に取り込み済みと同じく除外する
  （`DELETED_LINKED_IMPORT_SOURCE`）。スキーマを増やさないための流用で、`source` が違うので
  Gmail・car-careの重複判定とは交わらない。**削除を取り消して取り込み直したいときは、この行を消す**
...
**口座名は完全一致で引けなければ、末尾の括弧書きを1つ外して引き直す**（#419、`stripTrailingParenthetical`）。
Web版の一覧は出金元の口座名を画像の `alt` から読むため、「スマートレシート (自動連携)」のように
マスタの口座名へ表記が足されていると完全一致しない。**この表記は実データでまだ確かめられていない**
（2026-09-14時点。AIDEの送信がaide#272で止まっており、巡回結果が届いていない）。

- 「奨学金(日本学生支援機構)」のように括弧を含む口座が実在するので、**完全一致を必ず先に試す**。
  括弧を外すのは**Web版の名前の側だけ**で、マスタの口座名は加工しない
- **無効化した口座も衝突判定に含める**（計画レビューの指摘）。完全一致で引くのは有効な口座だけなので、
  無効化した「楽天カード(旧)」の明細がWeb版に出ると、有効な口座だけを見る判定では括弧を外した
  「楽天カード」へ寄り、**別の口座の明細として複製される**。Zaimの口座は実測で135件中103件が無効で
  めずらしくない（#335）。そのため、Web版の名前が無効な口座として実在するとき、括弧を外した名前が
  有効・無効の両方にあるときは引き直さない（`resolveAccountId`）
- それでも引けなかった口座名はプレビューに表記のまま出す（`unknownAccountNames`）。括弧書き以外の
  揺れだった場合は、ここに出た名前を見て突き合わせ方を直す

**サブPCにAIDEの巡回ジョブが入っていないと、この経路は黙って0件になる。** 2026-09-14まで
`aide-zaim-money-sync.timer` がサブPCに一度も導入されておらず、`GET /api/money/transactions` は
ずっと空だった（#419）。導入後も、AIDEの受け口が `zaim-money-snapshot` を知らず
`404 未知のキー` で捨てていた（aide#272）。候補が出ないときは、まずサブPCで
`journalctl --user -u aide-zaim-money-sync.service -n 20 --no-pager` を見る。
