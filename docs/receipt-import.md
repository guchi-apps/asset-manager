...
**置き換えの最後の1手（アプリの「置き換え」ボタン）はスマートフォンアプリ限定で、公開APIにも
Web版にも無い**（[公式手順](https://content.zaim.net/manuals/show/61) は iOS/Android の画面しか
示さない）。したがってコマンドでできるのは「候補として出るはずの明細を条件違いで用意する」ところ
までで、**候補に出たかどうかの確認は実機での操作が要る**。
...
### 実測で分かったこと（2026-08-31）

| | 結果 |
| --- | --- |
| 連携カード明細を公開APIで**取得**できるか | **できる。** 直近60日で「三井住友カード VISA」由来の支出320件が `GET /v2/home/money` から返る |
| 連携カード明細を公開APIで**更新**できるか | **できる。** 取得した値と同じ値を `PUT /v2/home/money/payment/:id` へ送ると成功する（`--check-edit`） |
| A: 反映待ち口座＋品目ありのAPI明細が候補に出るか | 実機での確認待ち |
| B: 連携カード口座＋品目ありのAPI明細が候補に出るか | 実機での確認待ち |

**「公開APIでは連携明細を取得・編集できない」は、このリポジトリでは成り立たない。** 既存の
置き換え候補（`lib/receipt-match.ts`）がカード明細を拾えているのも、内訳の自動振り分けが
カード明細へ内訳を書き戻せるのも同じ理由。

Cが成立したことで、**置き換えを経ずに「届いたカード明細へ直接品目を書き込む」経路が採れる**
可能性が出ている。利用者がこれまで手で行っていた運用（カード明細を開いて品目・コメントを
書き足す）と同じ形になる。品目（`name`）・店舗（`place`）まで書けるかは未確認で、
`updateZaimPaymentGenre` はカテゴリ・内訳しか送っていない。

**Asset Manager にPlaywright基盤は無い。** Zaimのログイン状態（storage state）と巡回スクリプトは
AIDE（`src/core/connectors/zaim/`）にあり、**サブPCのworkerでしか動かない**。Asset Manager 本体は
VPSで動くため、Web版の品目入力を自動化する案はAIDE側への実装が要り、このリポジトリだけでは
完結しない（`docs/zaim-auto-sync.md` の「責務の分担」）。

### 検証の手順

```bash
# 1. 下見。的にする連携カード明細と、登録する内容を表示するだけ（Zaimへ書き込まない）
npx -y tsx --env-file-if-exists=.env scripts/zaim-replace-probe.ts --card <カード口座id>
# 2. 条件違いの検証明細A・Bを登録する
npx -y tsx --env-file-if-exists=.env scripts/zaim-replace-probe.ts --card <カード口座id> --create
# 3. 連携明細をAPIで更新できるか（値を変えないので明細は変わらない）
npx -y tsx --env-file-if-exists=.env scripts/zaim-replace-probe.ts --card <カード口座id> --check-edit
# 4. 実機確認のあと、検証明細を削除する
...
- 的にするのは**品目もコメントも空で、同じ日付・金額の明細が他に無い**カード明細だけ。品目が
  入っている明細は人が書き足したか置き換え済みの可能性があり、「置き換え前」の状態を再現できない。
  日付・金額が重なる明細があると、候補に出たものがどのケース由来か判別できない
- **条件に合う的が無ければ黙って緩めず、エラーで止める。** 緩めると「候補に出なかった」の原因が
  条件なのか的の選び方なのか分からなくなる
