 ...
 *
 * **スマートレシート・Amazonなど、Zaimの自動連携が作った明細は `GET /v2/home/money` が返さない**
 * （Issue #379）。以前はコピー元にその口座を指定したルールが構造的に候補0件になっていたが、
 * #383でAIDE経由のZaim Web版の明細（`lib/zaim-web-source.ts`）を合流させたので、AIDEが巡回できて
 * いれば候補に出る。**ここの判定は合流元を区別しない**——どちらの明細も同じ形で渡ってくる。
